...
import {
  ...
  AddLiquidity,
  RemoveLiquidity
} from "../generated/DlpManager/DlpManager"

import {
  Distribute
} from "../generated/FeeDmxRewardDistributor/RewardDistributor"
...
export function handleAddLiquidity(event: AddLiquidity): void {
  _storeDlpStat(event.block.timestamp, event.params.dlpSupply, event.params.aumInUsdg)
}
...
export function handleDistributeEthToDmx(event: Distribute): void {
  let amount = event.params.amount
  let amountUsd = getTokenAmountUsd(WBTT, amount)
  let totalEntity = _getOrCreateDmxStat("total", "total")
  ...
  let entity = _getOrCreateDmxStat(id, "daily")
...
export function handleDistributeEsdmxToDmx(event: Distribute): void {
  let amount = event.params.amount
  let amountUsd = getTokenAmountUsd(DMX, amount)

  let totalEntity = _getOrCreateDmxStat("total", "total")
  totalEntity.distributedEsdmx += amount
  totalEntity.distributedEsdmxCumulative += amount
  totalEntity.distributedEsdmxUsd += amountUsd
  totalEntity.distributedEsdmxUsdCumulative += amountUsd
  ...
  entity.distributedEsdmx += amount
  entity.distributedEsdmxCumulative = totalEntity.distributedEthCumulative
  entity.distributedEsdmxUsd += amountUsd
  entity.distributedEsdmxUsdCumulative = totalEntity.distributedUsdCumulative
...
function _getOrCreateDmxStat(id: string, period: string): DmxStat {
  let entity = DmxStat.load(id)
  if (entity == null) {
    entity = new DmxStat(id)
    ...
    entity.distributedEsdmx = ZERO
    entity.distributedEsdmxCumulative = ZERO
    entity.distributedEsdmxUsd = ZERO
    entity.distributedEsdmxUsdCumulative = ZERO
    entity.period = period
  }
  return entity as DmxStat
}
...
function _getOrCreateDlpStat(id: string, period: string, periodTimestmap: i32): DlpStat {
  let entity = DlpStat.load(id)
  if (entity == null) {
    entity = new DlpStat(id)
    entity.period = period
    entity.dlpSupply = ZERO
  ...
  return entity as DlpStat
}

export function handleDistributeEthToDlp(event: Distribute): void {
  let amount = event.params.amount
  let amountUsd = getTokenAmountUsd(WBTT, amount)
  let periodTimestamp = parseInt(_getDayId(event.block.timestamp)) as i32

  let totalEntity = _getOrCreateDlpStat("total", "total", periodTimestamp)
  totalEntity.distributedEth += amount
  totalEntity.distributedEthCumulative += amount
  totalEntity.distributedUsd += amountUsd
  totalEntity.distributedUsdCumulative += amountUsd
  ...
  let id = periodTimestamp.toString() + ":daily"
  let entity = _getOrCreateDlpStat(id, "daily", periodTimestamp)
...
export function handleDistributeEsdmxToDlp(event: Distribute): void {
  let amount = event.params.amount
  let amountUsd = getTokenAmountUsd(DMX, amount)
  let periodTimestamp = parseInt(_getDayId(event.block.timestamp)) as i32
...
function _storeDlpStat(timestamp: BigInt, dlpSupply: BigInt, aumInUsdg: BigInt): void {
  let periodTimestamp = parseInt(_getDayId(timestamp)) as i32
  let totalEntity = _getOrCreateDlpStat("total", "total", periodTimestamp)
  totalEntity.aumInUsdg = aumInUsdg
  totalEntity.dlpSupply = dlpSupply
  totalEntity.save()

  let id = periodTimestamp.toString() + ":daily"
  let entity = _getOrCreateDlpStat(id, "daily", periodTimestamp)
  entity.aumInUsdg = aumInUsdg
  entity.dlpSupply = dlpSupply
  entity.save()
}
